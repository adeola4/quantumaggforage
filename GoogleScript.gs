...
function handleJoinClub(data) {
  ...
  const goalsText = data.goals || data.message || '';
  const row = [
  ...
  if (data.inquiryType) {
    fields.push({ name: 'Inquiry Type', value: data.inquiryType, inline: true });
  }
  if (data.experience) {
    fields.push({ name: 'Experience', value: data.experience, inline: true });
  }
  if (skills) {
    fields.push({ name: 'Skills', value: skills, inline: true });
  }
  if (data.maxTravel) {
    fields.push({ name: 'Max Travel', value: data.maxTravel, inline: true });
  }
  if (goalsText) {
    fields.push({ name: 'Message / Goals', value: truncate(goalsText, 500), inline: false });
  }

  sendDiscordNotification({
    color: 0x3E5F44,
    title: '🌿 New Club Member / Join Request',
    fields: fields
  });
...
function handleContact(data) {
  ...
  const msgText = data.message || data.goals || '';
  const row = [
    new Date().toISOString(),
    data.name || '',
    data.email || '',
  ...
  const fields = [
    ...
    { name: 'Email', value: data.email || 'N/A', inline: true },
    { name: 'Phone', value: data.phone || 'N/A', inline: true },
    { name: 'Type', value: data.inquiryType || 'General', inline: true },
  ];
  if (data.location) {
    fields.push({ name: 'Location', value: data.location, inline: true });
  }
  fields.push({ name: 'Message', value: truncate(msgText || 'N/A', 500), inline: false });
